...
Reviewed against `211d5c52` (branch `refactor_introduce_runners`).

### Must-fix

- M1 `_test_` stub has no `backup` block; import-time raise on `backup.backup_base_path` breaks every test suite.
  | `BACKUP_BASE_PATH` read from `backup.backup_base_path` in `shared/yamlsettings.py`, missing key raising at import like the runners do.
...
- D1 `Runner` drops `os` after using it to pick the path class. The future SSH handler (job script per OS) and the webapp will want it.
...
- D7 7.1 tests "each import-time validation error", but 2.2 names no callable; testing via re-import is brittle.
