...
- **`store_metrics` setting raw file status to DONE** — Status is now set by `finalize_raw_file_status` after all pipeline branches complete.
...
- **`finalize_raw_file_status` skipped-state bug**: When a job fails, `check_quanting_result` raises `AirflowSkipException`, marking `store_metrics` as "skipped". But `finalize_raw_file_status` only checks for "failed"/"upstream_failed" — not "skipped". A failed branch may be treated as successful. Deferred per decision.
