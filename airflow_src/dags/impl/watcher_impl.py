...
def get_unknown_raw_files(ti: TaskInstance, **kwargs) -> None:
    ...
    for raw_file in get_raw_files_by_names(list(raw_file_names_on_instrument)):
        if raw_file.instrument_file_status in (
            ...
            # renamed/disappeared files don't have a file size in the DB, but we want to consider their names for collision checking,
            # so we set a dummy size that will always trigger the collision logic
            raw_file_size = -1
        else:
