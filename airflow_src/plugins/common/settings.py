...
def get_path(path_type: str) -> Path:
    ...
    if path_type not in _SETTINGS["locations"]:
        raise KeyError(f"Setting {path_type} not found in locations.")
    return Path(_SETTINGS["locations"][path_type]["absolute_path"])
