...
class AcquisitionMonitor(BaseSensorOperator):
    ...
    def __init__(self, instrument_id: str, *args, **kwargs) -> None:
        ...
        self._raw_file_wrapper_factory: RawFileWrapperFactory | None = None
    ...
    def pre_execute(self, context: dict[str, any]) -> None:
        ...
        logging.info(
            f"Monitoring {self._raw_file_wrapper_factory.file_path_to_monitor_acquisition()}"
        )
    ...
    def poke(self, context: dict[str, any]) -> bool:
        ...
        if not self._raw_file_wrapper_factory.file_path_to_monitor_acquisition().exists():
        ...
        if time_since_last_check / 60 >= SIZE_CHECK_INTERVAL_M:
            size = get_file_size(
                self._raw_file_wrapper_factory.file_path_to_monitor_acquisition()
            )
