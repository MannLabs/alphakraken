...
class TestStaleStatusAlert:
    ...
    def test_check_should_handle_comprehensive_stale_scenarios(
        ...
        mock_job_fresh.updated_at_ = datetime(2024, 1, 15, 11, 50, 0)  # 10 minutes ago

        mock_job_stale = Mock()
        mock_job_stale.id = "monitor_job"
        mock_job_stale.entity_type = KrakenStatusEntities.JOB
        mock_job_stale.updated_at_ = datetime(2024, 1, 15, 11, 25, 0)  # 35 minutes ago
    ...
    def test_check_should_return_empty_when_all_fresh(
        ...
        entity_configs = [
            (
                ...
                datetime(2024, 1, 15, 11, 50, 0),
            ),  # 10 min ago
            (
                "instrument2",
                KrakenStatusEntities.INSTRUMENT,
                datetime(2024, 1, 15, 11, 55, 0),
            ),  # 5 min ago
            (
                "backup",
                KrakenStatusEntities.FILE_SYSTEM,
                datetime(2024, 1, 15, 11, 58, 0),
            ),  # 2 min ago
            (
                "output",
                KrakenStatusEntities.FILE_SYSTEM,
                datetime(2024, 1, 15, 11, 52, 0),
            ),  # 8 min ago
            (
                "processor",
                KrakenStatusEntities.JOB,
                datetime(2024, 1, 15, 11, 47, 0),
            ),  # 13 min ago
            (
                "file_remover",
                KrakenStatusEntities.JOB,
                datetime(2024, 1, 14, 14, 0, 0),
