...
class TestWebAppHealthAlert:
    ...
    def test_get_issues_should_return_empty_list_when_webapp_url_not_configured(
        self, mock_get_setting: Mock
    ) -> None:
        """Test that get_issues returns empty list when WEBAPP_URL is not configured."""
        # given
        mock_get_setting.side_effect = KeyError("WEBAPP_URL not found")
        alert = WebAppHealthAlert()
    ...
    def test_get_issues_should_return_empty_list_when_webapp_responds_200(
        self, mock_requests_get: Mock, mock_get_setting: Mock
    ) -> None:
        """Test that get_issues returns empty list when webapp responds with 200."""
        # given
        mock_get_setting.return_value = "http://localhost:8501"
        mock_response = Mock()
    ...
    def test_webapp_url_not_set_warning_logged(self, mock_get_setting: Mock) -> None:
        ...
        with patch(
            ...
            mock_warning.assert_any_call(
                "WEBAPP_URL not configured, skipping webapp health check"
            )
