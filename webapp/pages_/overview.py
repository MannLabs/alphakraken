...
def _show_csv_download(
    df: pd.DataFrame,
    *,
    label: str,
    file_name: str,
    display: DeltaGenerator,
    state_key: str,
) -> None:
    """Show a button that prepares the CSV for `df` and, once prepared, a button to download it.

    Preparing in a separate step keeps the (potentially large) CSV out of every rerun. Handing a
    callable to st.download_button instead would achieve the same, but Streamlit stores the file
    generated by such a callable without any session reference, so the file is garbage-collected
    before it is served and the download 404s (as of streamlit 1.61).
    """
    # the selection defines the CSV, not its content: columns like 'eta' change on every rerun.
    # the file name carries the timestamp of the underlying data.
    ...
    prepared = get_session_state(state_key)
    if prepared is None:
        return

    prepared_signature, csv_data = prepared
    if prepared_signature != signature:
        remove_session_state(state_key)
        return
    ...
    display.download_button(
        ...
        file_name=file_name,
        mime="text/csv",
        # downloading does not change any state, so there is no need to re-render the page
        on_click="ignore",
    )
...
def _display_table_and_plots(  # noqa: PLR0915,C901,PLR0912 (too many statements, too complex, too many branches)
    ...
    _show_csv_download(
        filtered_df,
        label=f"filtered table ({len(filtered_df)} entries)",
        file_name=f"{data_timestamp.strftime('AlphaKraken_%Y%m%d-%H%M%S_filtered')}.csv",
        display=c1,
        state_key=SessionStateKeys.PREPARED_CSV_FILTERED,
    )

    _show_csv_download(
        df,
        label=f"all data ({len(df)} entries)",
        file_name=f"{data_timestamp.strftime('AlphaKraken_%Y%m%d-%H%M%S_all')}.csv",
        display=c2,
        state_key=SessionStateKeys.PREPARED_CSV_ALL,
    )
